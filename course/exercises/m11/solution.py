"""
Module 11 Exercise — Observability & Feedback Loops (SOLUTION)
===============================================================
TODO filled in:
  performance_context = get_performance_history()
  (called INSIDE social_creative_node, not at module level)
...
# ─── SOLUTION: social_creative_node ──────────────────────────────────────────

def social_creative_node(state: PipelineState) -> dict:
    """
    Creative node with performance_context injected INSIDE the node.

    SOLUTION: performance_context = get_performance_history()
    ----------------------------------------------------------
    Key insight: calling get_performance_history() here (inside the node)
    means every invocation reads the latest DB state.

    If called at module level instead:
        performance_context = get_performance_history()   # at the top of the file
        def social_creative_node(state): ...

    Then ALL runs would share the same static snapshot from import time,
    and the feedback loop would never improve across runs.
    """
    global _history_called_inside_node

    # SOLUTION: call get_performance_history() inside the node
    performance_context = get_performance_history()   # ← fetched fresh every call

    # Mark that the call happened inside the node (for test inspection)
...
if __name__ == "__main__":
    ...
    summary_path = write_summary_json("demo-001", {"total_cost": 0.0012, "posts": 2})
